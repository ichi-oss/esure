...
export default function SubAdminPolicyDetailPage() {
  ...
  const handleSendEmail = async () => {
    ...
    try {
      const response = await fetch("/api/sub-admin/policies/send-email", {
        method: "POST",
        headers: {
        ...
        <main className="max-w-2xl mx-auto py-6 sm:px-6 lg:px-8">
          <Alert className="border-red-200 bg-red-50">
            <AlertDescription className="text-red-700">
              The requested policy could not be found or you don't have access
              to it.
            </AlertDescription>
          </Alert>
        </main>
          ...
          <div className="flex justify-between items-center py-6">
            ...
            <div className="flex items-center gap-3 flex-wrap">
              ...
              <Button
                variant="outline"
                onClick={() =>
                  router.push(`/sub-admin/policy/${policyId}/edit`)
                }
                className="flex items-center gap-2"
              >
